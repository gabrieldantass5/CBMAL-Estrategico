---
kanban-plugin: board
---

## 📖 GUIA / COMO USAR

- [ ] **Bem-vindo ao Workflow!** <br>Este quadro organiza nossas missões. Leia este guia para entender as colunas. 💡
- [ ] **Como mover?** <br>No Obsidian, clique e arraste os cartões entre as colunas conforme a tarefa evolui. 🖱️
- [ ] **Legenda de Símbolos:** <br>✅ = Concluído <br>⚠️ = Bloqueado/Aguardando <br>🚀 = Prioridade <br>📎 = Tem arquivo linkado
- [ ] **Dica de Ouro:** <br>Mantenha apenas 2 ou 3 tarefas em "Produção" para não se perder! 🎯
- [ ] **Link Úteis:** <br>[[00_START_HERE|Manual de Início]] <br>[[PAINEL_DE_CONTROLE|Acompanhamento Geral]]


## 📥 Backlog (Aguardando)
### Tarefas planejadas que ainda não começaram.

- [ ] Atualizar o PAP do SEI até sexta #foco_profundo #urgente
- [ ] Criar um dashboard para o Comandante Geral #foco_profundo
- [ ] Baixar a portaria nova de redação oficial #burocracia_sei
- [ ] Ajustar estética do Painel (Azul CBMAL) #estetica

- [ ] PAP 10: Benchmarking e Melhores Práticas
- [ ] PAP 11: Comunicação Estratégica (Visual)
- [ ] PAP 12: Capacitação de Pontos de Foco
- [ ] PAP 13: Gestão de Riscos Estratégicos
- [ ] PAP 14: Gestão de Projetos Estratégicos (MGP)
- [ ] PAP 15: Organização do Seminário Anual


## 🏗️ Em Curso (Produção)
### O que está sendo feito AGORA. Foque aqui!

- [ ] **Atualização da Legislação Tributária (JAN/26)**: Revisar proposta de taxas para cumprir meta do cronograma.
- [ ] **Dashboard do Comando Geral**: MVP de visualização executiva de indicadores.


## ⏳ Revisão / Bloqueado
### Pronto, mas depende de terceiros ou revisão.

- [ ] Aguardando Publicação do MGA
- [ ] Assinatura Final dos Instrumentos (Ten Gabriel Dantas)
- [ ] Publicação da Política PGE (SEI)


## ✅ Concluído (Janeiro/2026)
### Missões cumpridas (Histórico do mês).

- [x] **Otimização Radical do Ecossistema (21/01)**: Redução de 15 para 5 workflows, criação de regras atômicas e consolidação de documentação. ✅
- [x] **Deploy GitHub CBMAL-Estrategico (21/01)**: Repositório privado configurado com banner profissional e limpeza de histórico. ✅
- [x] **Upgrade Arquitetural Affaan Mustafa Style (21/01)**: Implementação de memória persistente, regras modulares e aprendizado contínuo. ✅
- [x] **Manual EPD-CBMAL v1.0**: 7 Capítulos + 5 Anexos (~1.100 linhas)
- [x] **Revisão Técnica Cruzada (Full Stack):** Política, Implementação, Monitoramento, Avaliação, Indicadores e Rastreabilidade. ✅
- [x] **Revisão Técnica Final:** Caderno de Indicadores (Ten Gabriel Dantas)
- [x] **Revisão Técnica Final:** Política Estratégica (Indexação e Sumário)
- [x] **PAP 09: Curadoria da Base de Conhecimento (IA)**
- [x] **PAP 08: Alinhamento de Planos Setoriais**
- [x] **PAP 07: Avaliação de Impacto e Valor Público**
- [x] **PAP 06: Redação de Notas Técnicas** (Padrão Oficial)
- [x] **PAP 05: Instrução de Adiamentos** (Processos SEI)
- [x] **PAP 04: Gestão de Contramedidas** (Tratamento de Crises)
- [x] **PAP 03: Condução do Rito RME** (Ritual de Monitoramento)
- [x] **PAP 02: Monitoramento e Dashboards** (Coleta de Dados)
- [x] **PAP 01: Gestão da Rotina e Kanban** (Guia do Substituto)
- [x] Atualização da Skill Especialista em PAPs (Padrão APO)
- [x] **Apresentação Tc Marcos Paulo** - Slides profissionais (45 min) ^completed-pres
- [x] Reorganização Geral do Repositório (Ativo vs Arquivo) ^completed-repo
- [x] Unificação dos Agentes de IA (Gemini Conductor CBMAL) ^completed-ai
- [x] Consolidação das Pastas de Configuração (.agent/.claude/.gemini) ^completed-config
- [x] Organização do Diretório Principal
- [x] Criação da Skill de Secretário
- [x] Relatório Diário Inicial
- [x] [[POLITICA_ESTRATEGICA_PARA_SEI_V_FINAL|Minuta Política Estratégica Final]]
- [x] [[CRONOGRAMA_ENTREGAS_2025_2026|Cronograma de Entregas 25-26]]
- [x] [[RELATORIO_EVOLUCAO_POLITICA_ESTRATEGICA|Relatório de Evolução da Política]]
- [x] **Impl. Ferramentas Power User**: Workflows `/handoff` e `/brain_dump` + Skill `analise_sessao`.
- [x] **Padronização de Diretórios**: Unificação das pastas 06 e 07 em `06_Diário_e_Memórias_IA`.
- [x] **PAP 10: Monitoramento e Gestão de Processos SEI** (Sincronizado com Referência Oficial) ✅
- [x] **Kit de Imagens Estratégicas**: 04 fluxogramas padronizados (Marinho/Vermelho) para Google Docs. ✅
- [x] **Upgrade Manual EPD v2.0**: Incluindo Arquitetura de 3 Camadas e DNA Digital (agents.md). ✅
- [x] **Ajuste BPMN**: Verticalização e correção de erros de renderização em todos os fluxogramas. ✅
- [x] **Integração Vercel Agent Browser**: Automação de navegador configurada para workspace. ✅

%% kanban:settings
{"kanban-plugin":"board","list-collapse":[false,false,false,false,false]}
%%
